...
export default async function DirectProductPage({ params }: ProductPageProps) {
    ...
    // Verify this is actually a direct product (no supplierId)
    if (product.supplierId) {
        // Redirect to the supplier product page instead
        return notFound();
    }

    const whatsapp = await db.getWhatsAppContact();

    // Fetch similar products in the same category/subcategory
    ...
    return (
        <SiteLayout activePath="/products">
            ...
            <section className="rounded-lg border-2 border-[#0b4f82] bg-white p-4 md:p-6 lg:p-8 shadow-sm">
                {/* Back to Category - Top Right, hidden on mobile (use breadcrumb instead) */}
                {product.category && product.subCategory && (
                    <div className="hidden md:flex justify-end mb-4">
                        <Link
                            href={`/suppliers?category=${encodeURIComponent(product.category)}&subCategory=${encodeURIComponent(product.subCategory)}`}
                        ...
                        <div>
                            ...
                            <div className="mt-4 md:mt-6 grid gap-4 md:gap-6 grid-cols-2 lg:grid-cols-4">
                                {similarProducts.map((similarProduct: any) => {
                                    ...
                                    // Determine the correct link based on whether it's a direct product or supplier product
                                    const productHref = similarProduct.supplierId
                                        ? `/suppliers/${similarProduct.supplierId}/products/${similarProduct.id}`
