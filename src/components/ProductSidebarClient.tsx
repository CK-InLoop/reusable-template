...
export default function ProductSidebarClient({
  ...
  useEffect(() => {
    const checkMobile = () => {
      setIsMobileDevice(isMobile || window.innerWidth < 1024 || 'ontouchstart' in window);
    };
    checkMobile();
  ...
  const onSubClick = (e: React.MouseEvent, category: string, sub: string) => {
    if (!isMobileDevice) return; // Let default link behavior work on desktop

    e.preventDefault();

    // If clicking the same subcategory, navigate to it
    if (selectedSubCategory?.category === category && selectedSubCategory?.sub === sub) {
      router.push(`/suppliers?category=${encodeURIComponent(category)}&subCategory=${encodeURIComponent(sub)}`);
      return;
    }

    // Otherwise, select it and fetch suppliers
    setSelectedSubCategory({ category, sub });
    fetchSuppliers(category, sub);
  };
  ...
  if (isMobileDevice) {
    return (
      <div ref={sidebarRef} className="flex flex-col w-full bg-white">
        {/* Categories Panel */}
        <aside className="flex flex-col w-full rounded-lg border border-slate-200 bg-white shadow-sm">
              ...
              return (
                <div key={section.name}>
                  <button
                    ...
                    className={`flex w-full items-center justify-between px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-[#0b4f82] min-h-[48px] ${isExpanded ? "bg-[#0b4f82] text-white" : ""}`}
                    aria-expanded={isExpanded}
                  >
                    <span>{formatText(section.name)}</span>
                    <svg
                      className={`h-4 w-4 flex-shrink-0 transition-transform ${isExpanded ? "rotate-90 text-white" : "text-slate-400"}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {/* Subcategories - Accordion Style for Mobile */}
                  {isExpanded && (
                    <div className="bg-slate-50 border-t border-slate-200">
                      {section.subCategories.map((sub) => {
                        if (sub.isHeading) {
                          return (
                            <div key={sub.name} className="px-4 pt-4 pb-1 text-xs font-bold text-yellow-600 uppercase tracking-wider">
                              {sub.name}
                            </div>
                          );
                        }

                        const isSelected = selectedSubCategory?.category === section.name && selectedSubCategory?.sub === sub.name;
        ...
        {/* Suppliers Panel - Shows when a subcategory is selected on mobile */}
        {selectedSubCategory && (
          <div className="mt-4 border border-[#0b4f82] bg-white shadow-lg rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-200 bg-[#0b4f82] flex items-center justify-between">
              <h3 className="text-sm font-semibold text-white">
                {formatText(selectedSubCategory.sub)}
              </h3>
              <button
                type="button"
                onClick={() => {
                  ...
                  <div className="border-t border-slate-200 pt-4 mt-4">
                    <div className="bg-slate-50 rounded-lg p-3 space-y-3">
                      ...
                      <textarea
                        ...
                        className="w-full h-16 px-2 py-2 text-xs border border-slate-200 rounded-md resize-none focus:outline-none focus:ring-1 focus:ring-[#0b4f82] focus:border-[#0b4f82]"
                      />
                      <div className="flex gap-2">
                        ...
                        <a
                          href={`mailto:sales@pakmon.com?subject=Inquiry about ${encodeURIComponent(selectedSubCategory.sub)}&body=${encodeURIComponent(inquiryMessage || `Hi, I'm interested in ${selectedSubCategory.sub}`)}`}
                          className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-[#0b4f82] text-white text-xs font-semibold rounded-md hover:bg-[#0a3d6b] transition"
                        >
