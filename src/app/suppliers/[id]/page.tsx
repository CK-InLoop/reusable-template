...
export default async function SupplierPage({
  ...
  return (
    <SiteLayout activePath="/products">
      {/* ... Supplier Card Section - Mobile Responsive ... */}
      <section className="mt-4 md:mt-8 rounded-lg border border-[#e2e8f0] bg-white p-4 md:p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
          {/* Profile Image */}
          <div className="relative h-20 w-32 md:h-28 md:w-48 flex-shrink-0 overflow-hidden rounded-lg bg-white border border-slate-100 shadow-sm mx-auto lg:mx-0">
          ...
          <div className="flex-1 flex flex-col gap-3">
            {/* Supplier Name */}
            <h1 className="text-lg md:text-xl font-bold tracking-tight text-[#0b4f82] text-center lg:text-left">
              {formatText(
                (supplier as any).companyName ||
                (supplier as any).name ||
                "Supplier"
            ...
            {/* Text Field + Buttons - Stacked on mobile, row on desktop */}
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              <textarea
                id="inquiry-message"
                placeholder="Have a question? Drop it here…"
                className="w-full lg:w-64 h-14 md:h-10 px-3 py-2 text-sm border border-slate-200 rounded-md resize-none focus:outline-none focus:ring-1 focus:ring-[#0b4f82] focus:border-[#0b4f82]"
              />
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 bg-[#0b4f82] text-white text-sm font-semibold rounded-md hover:bg-[#0a3d6b] transition whitespace-nowrap"
                >
      ...
      <section className="mt-8 md:mt-12">
        <div className="flex flex-col gap-4 md:gap-6 md:flex-row md:items-center md:justify-between mb-6 md:mb-8">
          <div>
            ...
            <h2 className="mt-1 md:mt-2 text-xl md:text-2xl lg:text-3xl font-bold text-[#0b4f82]">
              {(products as any[]).length} item{(products as any[]).length === 1 ? "" : "s"}
            </h2>
            <p className="mt-2 md:mt-3 max-w-2xl text-sm text-[#64748b]">
        ...
        {(products as any[]).length === 0 ? (
          <section className="mt-6 md:mt-8 rounded-lg border border-[#e2e8f0] bg-white p-4 md:p-6 text-sm text-[#64748b] shadow-sm">
          ...
          <div className="mt-6 md:mt-8 grid gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {(products as any[]).map((product: any) => (
              ...
              >
                <div className="relative h-36 md:h-48 overflow-hidden bg-white">
                ...
                <div className="flex flex-1 flex-col px-4 md:px-5 py-3 md:py-4">
                  <h3 className="text-sm md:text-base font-semibold text-[#171717]">
                    {formatText(product.title || product.name || "Product")}
                  </h3>
                  {product.shortDescription && (
                    <p className="mt-1.5 md:mt-2 text-xs md:text-sm text-[#64748b] line-clamp-2 md:line-clamp-3">
                      {formatText(product.shortDescription)}
                    </p>
                  )}
                  <div className="mt-3 md:mt-4 flex flex-wrap gap-1.5 md:gap-2 text-[10px] md:text-xs font-semibold uppercase tracking-wider text-[#64748b]">
                    {product.category && (
                      <span className="rounded-md border border-[#e2e8f0] px-2 md:px-4 py-1 md:py-2">
                  ...
                  <span className="mt-3 md:mt-4 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#0b4f82]">
